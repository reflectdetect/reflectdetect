# https://github.com/mstranne/py_matplotlib_helper/blob/main/src/3D_pose_plot.py
import math
...
def plotPose(ax, R, t, scale=np.array((1, 1, 1)), l_width=2, text=None):
    x_axis = np.array(([0, 0, 0], [1, 0, 0]), dtype="float64") * scale
    y_axis = np.array(([0, 0, 0], [0, 1, 0]), dtype="float64") * scale
    z_axis = np.array(([0, 0, 0], [0, 0, 1]), dtype="float64") * scale
